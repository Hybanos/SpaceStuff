...
void index_true_anomalies(Scene *scene, ECSTable &ecs) {
    ...
    for (auto &a : ecs.arch_iter(DRAWABLE_ORBIT, false)) {
        for (size_t i = 0; i < a->size(); i++) {
            ...
            TLE &tle = a->get_TLE(i);
            Orbit &orbit = a->get_Orbit(i);
            AnomalyIndex &anomaly_index = a->get_AnomalyIndex(i);
...
void compute_true_anomalies(Scene *scene, ECSTable &ecs) {
    for (auto &a : ecs.arch_iter(DRAWABLE_ORBIT, false)) {
        for (size_t i = 0; i < a->size(); i++) {
            ...
            double days_since_epoch = (scene->get_time().time_since_epoch().count() / 1e9 - epoch) / (24 * 3600);
...
void compute_pos_along_orbit(Scene *scene, ECSTable &ecs) {
    for (auto &a : ecs.arch_iter(DRAWABLE_ORBIT, false)) {
        for (size_t i = 0; i < a->size(); i++) {
            ...
            Orbit &orbit = a->get_Orbit(i);
            Rotation &rota = a->get_Rotation(i);
